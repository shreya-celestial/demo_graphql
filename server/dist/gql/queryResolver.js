"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.queryResolver = void 0;
const login_1 = __importDefault(require("../utils/login"));
const signup_1 = __importDefault(require("../utils/signup"));
const getData_1 = __importDefault(require("../utils/getData"));
exports.queryResolver = {
    someQuery(_, args, context) {
        return __awaiter(this, void 0, void 0, function* () {
            console.log(context.access);
            if (process.env.HASURA_KEY) {
                args.queryVars = JSON.parse(args.queryVars);
                if (args.queryName === 'SignInUser') {
                    const data = yield (0, login_1.default)(args.queryVars);
                    return JSON.stringify(data);
                }
                if (args.queryName === 'SignUpUser') {
                    const data = yield (0, signup_1.default)(args.queryVars, args.gqlQuery.toString());
                    return JSON.stringify(data);
                }
                const graphql = {
                    query: args.gqlQuery.toString(),
                    variables: args.queryVars
                };
                const data = yield (0, getData_1.default)(graphql);
                return JSON.stringify(data);
            }
            return null;
        });
    }
};
